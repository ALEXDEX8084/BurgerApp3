...
namespace MvvmBurgerApp.ViewModels
{
    public class BurgerViewModel : INotifyPropertyChanged
    {
        ...
        public BurgerViewModel()
        {
            ...
            Burger.Onion = new Additionally("Лук", 10, 0);
            Burger.Pickle = new Additionally("Огурец", 15, 0);
            //{
        ...
        private void RecalTotal()
        {
            if (Count > 0)
            {
                Total = Count * Price + SalatCount * Burger.Salat.Price + TomatCount * Burger.Tomat.Price + OnionCount * Burger.Onion.Price  + PickleCount * Burger.Pickle.Price;
            }
        }
        ...
        {
            get { return Burger.Pickle.Count; }
            set
            {
                if (Burger.Pickle.Count != value)
                {
                    Burger.Pickle.Count = value;
                    OnPropertyChanged("PickleCount");
                }
            }
        }
